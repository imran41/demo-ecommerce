...
export default function ForgotPasswordPage() {
  ...
  // 2: Enter & Verify 6-digit OTP (Mock Mode only)
  // 3: Input New Password & Confirm (Mock Mode only)
  // 4: Success message for real Email link reset
  ...
  // Step 1: Request OTP or Link Reset
  const handleRequestOtp = async (e) => {
    ...
    setLoading(true);
    try {
      // Real Live Authentication Mode (Firebase / Supabase)
      if (!authService.isMock) {
        if (method !== 'email') {
          showToast('SMS recovery is only supported in mock mode. Please use Email.', 'error');
          setLoading(false);
          return;
        }
      ...
      // Mock Mode (generates OTP code locally)
      ...
      showToast(err.message || 'Failed to trigger reset. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4 py-12 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md bg-white/10 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl space-y-6 text-white">
        ...
        {/* STEP 4: Success message for real Email link reset */}
        {step === 4 && (
          <div className="space-y-4 text-center">
            <div className="mx-auto h-12 w-12 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400 mb-2">
              <Mail className="h-6 w-6" />
            </div>
            <h3 className="text-md font-bold">Recovery Link Sent</h3>
            <p className="text-xxs text-slate-300 leading-relaxed">
              We have sent a secure password reset link to <strong>{email}</strong>. 
              Please check your inbox (and spam folder) and follow the instructions to set your new password.
            </p>
            <Link
              href="/login"
              className="w-full h-11 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold flex items-center justify-center gap-2 transition-colors text-xs mt-4"
            >
              Return to Login
            </Link>
          </div>

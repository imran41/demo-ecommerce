...
export const productService = {
  ...
  async getProducts(filters = {}) {
    if (isFirebaseConfigured) {
      ...
      // Client-side pricing filtering (overcomes composite index creation dependencies)
      ...
      // Search filters
      if (filters.search) {
        const queryVal = filters.search.toLowerCase();
        list = list.filter(p => 
          p.name.toLowerCase().includes(queryVal) || 
          p.description.toLowerCase().includes(queryVal) ||
          p.brand.toLowerCase().includes(queryVal) ||
          p.category.toLowerCase().includes(queryVal)
        );
      }
      ...
      list = list.map(p => ({ ...p, rating: p.rating || 4.5 }));
      if (filters.rating) {
        list = list.filter(p => p.rating >= parseFloat(filters.rating));
      }
  ...
  async getProductById(id) {
    if (isFirebaseConfigured) {
      ...
      return { id: docSnap.id, ...data, rating: data.rating || 4.5 };
    } else if (isSupabaseConfigured) {
